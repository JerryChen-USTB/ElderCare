<template>
  <view class="new-schedule-container">
    <!-- uv-calendars 日历组件 -->
    ...
    <view class="form-container">
      ...
      <view class="form-item">
        ...
        <picker 
          @change="bindRepeatChange" 
          :value="repeatIndex" 
          :range="repeatTypes"
          range-key="name"
        >
          <view class="picker-item">
            <text>{{repeatTypes[repeatIndex].name}}</text>
            <uni-icons type="arrowright" size="16" color="#999"></uni-icons>
          </view>
        </picker>
      </view>
...
export default {
  data() {
    return {
      ...
      customLocation: '',
      repeatTypes: [
        { name: '一次性日程', value: 'none' },
        { name: '每天重复', value: 'daily' },
        { name: '每周重复', value: 'weekly' },
        { name: '每月重复', value: 'monthly' }
      ],
      repeatIndex: 0
    };
  },
    ...
    async submitSchedule() {
      ...
      try {
        ...
        const newScheduleData = {
          ...
          status: 'pending', // 新建的日程默认为待处理状态
          repeatType: this.repeatTypes[this.repeatIndex].value
        };
        
        // 调用后端API创建日程
        const response = await uni.request({
          url: `${request.BASE_URL}/api/schedule/create`,
          method: 'POST',
          header: {
        ...
        if (response.statusCode === 200) {
          if (response.data && response.data.success) {
            ...
            uni.showToast({
              title: '添加失败：' + (response.data ? response.data.message : '未知错误'),
              icon: 'none'
            });
          }
        } else {
